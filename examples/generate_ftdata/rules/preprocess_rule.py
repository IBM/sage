...
def make_sequence(ctx: AnsibleRunContext, current_depth, current_id) -> list:
    ...
    for node in ctx:
        ...
        if node.type == RunTargetType.Role:
            ...
            node_data["description"] = rolecall.spec.metadata.get("galaxy_info", {}).get("description", "")
        elif node.type == RunTargetType.TaskFile:
            pass
        elif node.type == RunTargetType.Task:
            taskcall = node
            module = taskcall.get_annotation("module.correct_fqcn", "")
            if module == "":
                module = getattr(taskcall.spec, "module")
            node_data["module"] = module
            node_data["module_options"] = getattr(taskcall.spec, "module_options")
...
class PreProcessingRule(Rule):
    rule_id: str = "PP001"
    description: str = "prepare context"
    enabled: bool = True
    name: str = "GetContext"
    version: str = "v0.0.1"
    ...
    def process(self, ctx: AnsibleRunContext):
        ...
        train["ari_node_id"] = current_id
        train["ari_node_depth"] = current_depth
