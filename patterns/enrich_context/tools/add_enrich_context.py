...
class Mapping(object):
    ...
    def run(self, ari_file, wisdom_file, src_type, repo_name, ftdata_type=""):
        ari_ftdata = load_json_data(ari_file)


        if not ftdata_type:
            ftdata_type = src_type
        wisdom_ftdata = load_json_data(wisdom_file, ftdata_type, repo_name)
        print(f"comparing tasks: ari {len(ari_ftdata)} wisdom {(len(wisdom_ftdata))}")

        matched_list, only_org_ftdata = self.separate_wisdom_data(wisdom_ftdata, ari_ftdata)

        modified_wisdom_ftdata = self.add_dense_context(matched_list)
        os.makedirs(os.path.join(self.output_dir, ftdata_type), exist_ok=True)
        export_result(os.path.join(self.output_dir, ftdata_type, "_modified_ftdata.json"), modified_wisdom_ftdata)
        export_result(os.path.join(self.output_dir, ftdata_type, "_only_org_ftdata.json"), only_org_ftdata)
        all = modified_wisdom_ftdata + only_org_ftdata
        export_result(os.path.join(self.output_dir, ftdata_type, "enriched_ftdata.json"), all)
        return
    ...
    def separate_wisdom_data(self, wisdom_list, ari_list):
        ...
        for i, w_item in enumerate(wisdom_list):
            ...
            w_module_short_name = w_module_name.split(".")[-1]
            ...
            for a_item in ari_list:
                ...
                a_module_short_name = a_module_name.split(".")[-1]
                ...
                if w_path != "" and w_path != a_path:
                    continue
                if w_module_short_name != a_module_short_name:
                    continue

                # exact match
                if w_task_spec_str == a_task_spec_str:
                    matched_ari = {"wisdom": w_item, "ari": a_item}
                    is_spec_match = True
                    break
                # similarity
                s = Levenshtein.ratio(w_task_spec_str, a_task_spec_str)
                candidates.append({
                    "similarity": s,
                    "ari": a_item
                })
            ...
            if is_spec_match:
                match_list.append(matched_ari)
            elif len(candidates) != 0:
                sorted_candidates = sorted(candidates, key=lambda x: x['similarity'], reverse=True)
                if sorted_candidates[0]["similarity"] > 0.6:
                    match_list.append({"wisdom": w_item, "ari": sorted_candidates[0]["ari"]})
                else:
                    only_org_ftdata.append(w_item)
            else:
                only_org_ftdata.append(w_item)

        return match_list, only_org_ftdata
...
if __name__ == '__main__':
    ...
    tmp_sage_ftdata = os.path.join(sage_dir, "_tmp-ftdata.json") # with correct path
